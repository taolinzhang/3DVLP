...
class ScannetDatasetConfig(object):
    def __init__(self):
        ...
        self.mean_size_arr_tensor = torch.tensor(self.mean_size_arr).cuda()
    ...
    def class2angle_batch_tensor(self, pred_cls, residual, to_label_format=True):
        ''' Inverse function to angle2class.

        As ScanNet only has axis-alined boxes so angles are always 0. '''
        return torch.zeros(pred_cls.shape[0])

    def class2size_batch_tensor(self, pred_cls, residual):
        ''' Inverse function to size2class '''
        return self.mean_size_arr_tensor[pred_cls] + residual

    def param2obb_batch_tensor(self, center, heading_class, heading_residual, size_class, size_residual):
        heading_angle = self.class2angle_batch_tensor(heading_class, heading_residual)
        box_size = self.class2size_batch_tensor(size_class, size_residual)
