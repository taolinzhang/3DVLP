...
SCANREFER_ORGANIZED = json.load(
    open(os.path.join(CONF.PATH.DATA, "ScanRefer_filtered_organized.json")))
...
def feed_scene_cap(model, device, dataset, dataloader, phase, folder,
                   use_tf=False, is_eval=True, max_len=CONF.TRAIN.MAX_DES_LEN, save_interm=False, min_iou=CONF.EVAL.MIN_IOU_THRESHOLD, organized=SCANREFER_ORGANIZED, tokenizer=None):
    candidates = {}
    intermediates = {}
    ...
    for data_dict in dataloader:
        ...
        with torch.no_grad():
            ...
            data_dict = get_scene_cap_loss(
                data_dict, device, DC, weights=dataset.weights, detection=True, caption=False)
        ...
        detected_object_ids = torch.gather(
            data_dict["scene_object_ids"], 1, data_dict["object_assignment"])
            ...
            for prop_id in range(num_proposals):
                if nms_masks[batch_id, prop_id] == 1 and good_bbox_masks[batch_id, prop_id] == 1:
                    ...
                    caption_decoded = decode_caption(
                        tokenizer, captions[batch_id, prop_id])
                    # if is_eval:
                    #     print(scene_id,object_id)
                    #     print(caption_decoded,flush=True)

                    # print(scene_id, object_id)
                    # try:
                    if object_id not in organized[scene_id].keys():
                        continue
                    ...
                    if save_interm:
                        ...
                        intermediates[scene_id][object_id]["box_corner"] = detected_bbox_corners[batch_id, prop_id].cpu(
                        ).numpy().tolist()
...
def eval_cap(model, device, dataset, dataloader, phase, folder,
             ...
             mode="scene", save_interm=False, no_caption=False, no_classify=False, min_iou=CONF.EVAL.MIN_IOU_THRESHOLD, tokenizer=None):
    if no_caption:
        bleu = 0
        cider = 0
        rouge = 0
        ...
        if mode == "scene":
            candidates = feed_scene_cap(model, device, dataset, dataloader, phase, folder, use_tf,
                                        is_eval, max_len, save_interm, min_iou, organized=organized, tokenizer=tokenizer)
        elif mode == "object":
            candidates, cls_acc = feed_object_cap(
                model, device, dataset, dataloader, phase, folder, use_tf, is_eval, max_len)
        elif mode == "oracle":
            candidates = feed_oracle_cap(
                model, device, dataset, dataloader, phase, folder, use_tf, is_eval, max_len)
        else:
        ...
        if save_interm:
            ...
            interm = update_interm(
                interm, candidates, bleu, cider, rouge, meteor)
