...
class ScannetQADataset(Dataset):
    def __init__(self, scanqa, scanqa_all_scene,
        ...
        if split == 'train':
            ...
            if use_unanswerable:
                ...
                self.scanqa = [data for data in scanqa if len(
                    set(data['answers']) & set(answer_cands)) > 0]
        ...
        self.scene_id_to_number = {scene_id: int(
            ''.join(re.sub('scene', '', scene_id).split('_'))) for scene_id in scene_ids}
        self.scene_number_to_id = {v: k for k,
                                   v in self.scene_id_to_number.items()}
        ...
        if tokenizer is None:
            ...
            for record in self.scanqa:
                record.update(token=tokenizer(
                    record['question'], return_tensors='np'))
    ...
    def __getitem__(self, idx):
        ...
        if self.split != 'test':
            ...
            object_names = [' '.join(object_name.split('_'))
                            for object_name in self.scanqa[idx]['object_names']]
        else:
            object_ids = None
            object_names = None
        ...
        if self.answer_counter is not None:
            ...
            for answer, answer_ind in zip(answers, answer_inds):
                ...
                answer_cat_score = get_answer_score(
                    self.answer_counter.get(answer, 0))
        ...
        if self.use_bert_embeds:
            ...
            lang_feat['attention_mask'] = lang_feat['attention_mask'].astype(
                np.float32)
            if 'token_type_ids' in lang_feat:
                lang_feat['token_type_ids'] = lang_feat['token_type_ids'].astype(
                    np.int64)
        ...
        if self.use_normal:
            ...
            point_cloud = np.concatenate(
                [point_cloud, normals], 1)  # p (50000, 7)
        ...
        if self.use_multiview:
            ...
            if pid not in self.multiview_data:
                self.multiview_data[pid] = h5py.File(
                    MULTIVIEW_DATA + '.hdf5', 'r', libver='latest')
        ...
        data_dict['vote_label_mask'] = point_votes_mask.astype(
            np.int64)  # point_obj_mask (gf3d)
        data_dict['scan_idx'] = np.array(idx).astype(np.int64)
        data_dict['pcl_color'] = pcl_color
        # (MAX_NUM_OBJ,) # 0/1 reference labels for each object bbox
        data_dict['ref_box_label'] = ref_box_label.astype(np.int64)

        data_dict['ref_center_label'] = ref_center_label.astype(
            np.float32)  # (3,)
        data_dict['ref_heading_class_label'] = np.array(
            int(ref_heading_class_label)).astype(np.int64)  # (MAX_NUM_OBJ,)
        data_dict['ref_heading_residual_label'] = np.array(
            int(ref_heading_residual_label)).astype(np.int64)  # (MAX_NUM_OBJ,)
        data_dict['ref_size_class_label'] = np.array(
            int(ref_size_class_label)).astype(np.int64)  # (MAX_NUM_OBJ,)
        ...
        data_dict['scene_id'] = np.array(
            int(self.scene_id_to_number[scene_id])).astype(np.int64)
        if type(question_id) == str:
            data_dict['question_id'] = np.array(
                int(question_id.split('-')[-1])).astype(np.int64)
        else:
            data_dict['question_id'] = np.array(
                int(question_id)).astype(np.int64)
        data_dict['pcl_color'] = pcl_color
        data_dict['load_time'] = time.time() - start
        data_dict['answer_cat'] = np.array(
            int(answer_cat)).astype(np.int64)  # 1
        ...
        if self.answer_cls_loss == 'bce' and self.answer_counter is not None:
            data_dict['answer_cat_scores'] = answer_cat_scores.astype(
                np.float32)  # num_answers

        data_dict["input_ids"] = np.expand_dims(lang_feat["input_ids"], axis=0)
        data_dict["token_type_ids"] = np.expand_dims(
            lang_feat["token_type_ids"], axis=0)
        data_dict["bert_attention_mask"] = np.expand_dims(
            lang_feat["attention_mask"], axis=0)
        data_dict["istrain"] = istrain
        data_dict["lang_num"] = np.array(1).astype(np.int64)
    ...
    def _tranform_text_bert(self, token_type='token'):
        ...
        for data in self.scanqa:
            ...
            if 'token_type_ids' in data[token_type]:
                ...
                padded_token_type_ids = pad_tokens(
                    data[token_type]['token_type_ids'])
                padded_attention_mask = pad_tokens(
                    data[token_type]['attention_mask'])
                # store
    ...
    def _load_data(self):
        ...
        self.scene_list = sorted(
            list(set([data['scene_id'] for data in self.scanqa])))
