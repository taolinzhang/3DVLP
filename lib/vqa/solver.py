...
from lib.loss_helper.loss_joint import get_joint_loss as get_loss
import pandas as pd
from lib.pointnet2.pytorch_utils import BNMomentumScheduler
from utils.eta import decode_eta
from lib.vqa.eval_helper import get_eval
...
class Solver():
    ...
    def _backward(self):
        ...
        if self.max_grad_norm is not None and self.max_grad_norm > 0:
            nn.utils.clip_grad_value_(
                self.model.parameters(), clip_value=self.max_grad_norm)
    ...
    def _feed(self, dataloader, phase, epoch_id):
        ...
        for data_dict in dataloader:
            ...
            answerable_rate = self.answerable_data_size[phase] / \
                self.all_data_size[phase]

            if "pred_langs" in data_dict:
                self.log[phase]["pred_lang"] += data_dict["pred_langs"].argmax(
                    axis=1).tolist()

            if "pred_answers" in data_dict:
                self.log[phase]["pred_answer"] += data_dict["pred_answers"].tolist()

            self.log[phase]["scene_id"] += [scene_number_to_id[scene_number]
                                            for scene_number in data_dict["scene_id"].tolist()]
        ...
        if phase == "val":
            ...
            if cur_best > self.best[self.cur_criterion]:
                self._log("best val_{} achieved: {}".format(
                    self.cur_criterion, cur_best))
                self._log("current train_loss: {}".format(
                    np.mean(self.log["train"]["loss"])))
                self._log("current val_loss: {}".format(
                    np.mean(self.log["val"]["loss"])))
                self._log("current val_iou_rate_0.5: {}".format(
                    np.mean(self.log["val"]["iou_rate_0.5"])))
                self._log("current val_iou_rate_0.5: {}".format(
                    np.mean(self.log["val"]["iou_rate_0.5"])))
                ...
                if "pred_answer" in self.log[phase]:
                    ...
                    pred_answers = [self.dataloader["val"].dataset.answer_vocab.itos(
                        pred_answer_idx) for pred_answer_idx in pred_answer_idxs]

                    qa_id_df = pd.DataFrame(
                        [self.log[phase]["scene_id"], self.log[phase]["question_id"]]).T
                    qa_id_df.columns = ["scene_id", "question_id"]
                    if len(self.log[phase]["pred_lang"]) != 0:
                        pred_lang_idxs = self.log[phase]["pred_lang"]

                        # dataloader.iterable
                        pred_langs = [self.dataloader["val"].dataset.label2raw[pred_lang_idx]
                                      for pred_lang_idx in pred_lang_idxs]
                        pred_ansewr_df = pd.DataFrame(
                            [pred_lang_idxs, pred_langs, pred_answer_idxs, pred_answers]).T
                        pred_ansewr_df.columns = [
                            "pred_lang_idx", "pred_lang", "pred_answer_idx", "pred_answer"]
                    else:
                        pred_ansewr_df = pd.DataFrame(
                            [pred_answer_idxs, pred_answers]).T
                        pred_ansewr_df.columns = [
                            "pred_answer_idx", "pred_answer"]

                    # save pred_answers
                    pred_ansewr_df = pd.concat(
                        [qa_id_df, pred_ansewr_df], axis=1)
                    pred_ansewr_df.to_csv(os.path.join(
                        model_root, "best_val_pred_answers.csv"), index=False)
    ...
    def _dump_log(self, phase):
        for loss_or_score in ["loss", "score"]:
            for key in LOG_SCORE_KEYS[loss_or_score]:
                ...
                wandb.log({"{}/{}/{}".format(phase, loss_or_score,
                          key): value}, step=self._global_iter_id)
    ...
    def _train_report(self, epoch_id):
        ...
        eta_sec = (self._total_iter["train"] -
                   self._global_iter_id - 1) * mean_train_time
        ...
        phase = "train"
        for key in LOG_SCORE_KEYS["loss"] + LOG_SCORE_KEYS["score"]:
            iter_report_dic[phase+"_"+re.sub('0.', '', key)] = round(
                np.mean([v for v in self.log[phase][key]]), 5)
        ...
        iter_report_dic["mean_backward_time"] = round(
            np.mean(backward_time), 5)
    ...
    def _epoch_report(self, epoch_id):
        ...
        for phase in ["train", "val"]:
            for key in LOG_SCORE_KEYS["loss"] + LOG_SCORE_KEYS["score"]:
                epoch_report_dic[phase + "_" + re.sub('0.', '', key)] = round(
                    np.mean([v for v in self.log[phase][key]]), 5)
    ...
    def _best_report(self):
        ...
        best_report_dic = {re.sub('0.', '', k): v for k,
                           v in self.best.items()}
        ...
        best_report_file = os.path.join(
            CONF.PATH.OUTPUT, self.stamp, "best.txt")
