...
def select_target(data_dict):
    ...
    pred_bbox = data_dict["pred_bbox_corner"]
    ...
    for i in range(batch_size):
        ...
        gt_bbox_batch = gt_bbox[i].unsqueeze(0).repeat(
            num_proposals, 1, 1)  # num_proposals, 8, 3
...
def select_multi_target(data_dict):
    ...
    # ground truth bbox
    # batch_size, lang_num_max, 8, 3
    gt_bbox = data_dict["ref_box_corner_label_list"].float()
    batch_size, len_nun_max, box_size = gt_bbox.shape[:3]
    # print("word_embs", word_embs[0][0][0][:5], word_embs[0][1][0][:5])
    # batch_size * lang_num_max, 8, 3
    gt_bbox = gt_bbox.reshape(batch_size * len_nun_max, box_size, -1)
    ...
    target_ious = torch.FloatTensor(
        target_ious).cuda()  # batch_size * lang_num_max
...
class TopDownSceneCaptionModule(nn.Module):
    def __init__(self, vocabulary, embeddings, emb_size=768, feat_size=128, hidden_size=512, num_proposals=256,
        ...
        self.num_vocabs = self.bert_config.vocab_size
        ...
        self.map_previous = nn.Sequential(
            nn.Linear(hidden_size + feat_size + emb_size,
                      128),  # emb_size hidden_size
            nn.ReLU()
        )
    ...
    def _step(self, step_input, target_feat, obj_feats, hidden, object_masks):
        ...
        # print("step input",step_input.shape)
        # print("hidden",hidden.shape)
        # print("target feat",target_feat.shape)
        ...
        proposal_feats = F.relu(self.obj_fc(obj_feats))
        proposal_feats = self.obj_dropout(proposal_feats)
        proposal_feats = self.obj_layer_norm(proposal_feats)

        # batch_size, num_proposals, hidden_size
        combined = self.map_feat(obj_feats)
        ...
        if len(step_input.shape) == 2:
            lang_input = self.dec_att2(lang_input.unsqueeze(
                1), proposal_feats, proposal_feats).squeeze(1)
        else:
            lang_input = self.dec_att2(
                lang_input, proposal_feats, proposal_feats)
    ...
    def _query_locals(self, data_dict, target_ids, object_masks, include_self=True, overlay_threshold=CONF.TRAIN.OVERLAID_THRESHOLD):
        ...
        target_centers = torch.gather(
            centers, 1, target_ids.view(-1, 1, 1).repeat(1, 1, 3))  # batch_size, 1, 3
        target_corners = torch.gather(
            corners, 1, target_ids.view(-1, 1, 1, 1).repeat(1, 1, 8, 3))  # batch_size, 1, 8, 3

        # get the distance
        if self.query_mode == "center":
            pc_dist = self._nn_distance(target_centers, centers).squeeze(
                1)  # batch_size, num_proposals
        elif self.query_mode == "corner":
            pc_dist = self._nn_distance(target_corners.squeeze(
                1), centers)  # batch_size, 8, num_proposals
            pc_dist, _ = torch.min(pc_dist, dim=1)  # batch_size, num_proposals
        else:
            raise ValueError(
                "invalid distance mode, choice: [\"center\", \"corner\"]")

        # mask out invalid objects
        # distance to invalid objects: infinity
        pc_dist.masked_fill_(object_masks == 0, float('1e30'))
        ...
        # distance to overlaid objects: infinity
        pc_dist.masked_fill_(overlaid_masks, float('1e30'))
        ...
        # distance to themselves: 0 or infinity
        pc_dist.masked_fill_(self_masks == 1, self_dist)

        # get the top-k object ids
        # batch_size, num_locals
        _, topk_ids = torch.topk(
            pc_dist, self.num_locals, largest=False, dim=1)
    ...
    def _query_multi_locals(self, data_dict, target_ids, object_masks, include_self=True, overlay_threshold=CONF.TRAIN.OVERLAID_THRESHOLD):
        ...
        # decode target box info
        # batch_size*len_nun_max, 1, 3
        target_centers = torch.gather(
            centers, 1, target_ids.view(-1, 1, 1).repeat(1, 1, 3))
        # batch_size*len_nun_max, 1, 8, 3
        target_corners = torch.gather(
            corners, 1, target_ids.view(-1, 1, 1, 1).repeat(1, 1, 8, 3))

        # get the distance
        if self.query_mode == "center":
            pc_dist = self._nn_distance(target_centers, centers).squeeze(
                1)  # batch_size*len_nun_max, num_proposals
        elif self.query_mode == "corner":
            # batch_size*len_nun_max, 8, num_proposals
            pc_dist = self._nn_distance(target_corners.squeeze(1), centers)
            # batch_size*len_nun_max, num_proposals
            pc_dist, _ = torch.min(pc_dist, dim=1)
        else:
            raise ValueError(
                "invalid distance mode, choice: [\"center\", \"corner\"]")
        ...
        # include the target objects themselves
        self_dist = 0 if include_self else float('1e30')
        self_masks = torch.zeros(
            batch_size*len_nun_max, self.num_proposals).cuda()
        self_masks.scatter_(1, target_ids.view(-1, 1), 1)
        # distance to themselves: 0 or infinity
        pc_dist.masked_fill_(self_masks == 1, self_dist)
    ...
    def forward(self, data_dict, use_tf=True, is_eval=False, max_len=CONF.TRAIN.MAX_DES_LEN, bert_embeds=None):
        if not is_eval:
            data_dict = self._forward_sample_batch(
                data_dict, max_len, bert_embeds=bert_embeds)
        else:
            data_dict = self._forward_scene_batch(
                data_dict, use_tf, max_len, bert_embeds=bert_embeds)
    ...
    def _create_adjacent_mat(self, data_dict, object_masks):
        ...
        for obj_id in range(num_objects):
            target_ids = torch.LongTensor(
                [obj_id for _ in range(batch_size)]).cuda()
            adjacent_entry = self._query_locals(
                data_dict, target_ids, object_masks, include_self=False)  # batch_size, num_objects
    ...
    def _add_relation_feat(self, data_dict, obj_feats, target_ids):
        # batch_size, num_proposals, num_locals, feat_size
        rel_feats = data_dict["edge_feature"]
        ...
        # batch_size, num_proposals, num_proposals
        adjacent_mat = data_dict["adjacent_mat"]
        ...
        rel_masks = rel_indices.unsqueeze(-1).repeat(1, 1, self.feat_size) == 1
        scattered_rel_feats = torch.zeros(obj_feats.shape).cuda().masked_scatter(
            rel_masks, rel_feats)  # batch_size, num_proposals, feat_size
    ...
    def _expand_object_mask(self, data_dict, object_masks, num_extra):
        ...
        # batch_size, num_objects + num_extra
        object_masks = torch.cat([object_masks, exp_masks], dim=1)

        return object_masks

    def get_local_feat(self, data_dict, obj_feats, object_masks):
        batch_size, num_proposal, feature_size = obj_feats.shape
        obj_masks = object_masks.bool().view(
            batch_size * num_proposal)  # batch_size * num_proposals
        ...
        # batch_size * num_local, feature_size
        obj_features = obj_features[obj_masks, :]
        obj_features = obj_features.view(
            batch_size, self.num_locals, feature_size)

        return obj_features

    def target_feat_aug(self, data_dict, target_ids, target_feats, obj_feats):
        # batch_size * len_nun_max, num_proposal, 128
        B, num_proposal, obj_feature_size = obj_feats.shape
        len_nun_max = data_dict["input_ids"].shape[1]
        batch_size = B // len_nun_max

        obj_feats = obj_feats.view(
            batch_size, len_nun_max, num_proposal, obj_feature_size)
        pred_cluster_labels = target_ids.view(
            batch_size, len_nun_max, 1)  # B len_nun_max 1
        """
        ...
        center = center[:, None, :, :].repeat(1, len_nun_max, 1, 1)
        pred_obj_center = torch.gather(center, 2, pred_cluster_labels.unsqueeze(3).repeat(
            1, 1, 1, 3)).view(batch_size * len_nun_max, 1, -1)  # B*len_nun_max 1 3
        center = center.view(batch_size * len_nun_max, num_proposal, -1)
        # ind: B*len_nun_max 1 k
        dist, ind = knn_distance(
            center[:, :, 0:3], pred_obj_center[:, :, 0:3], k=self.k)
        ind = ind.permute(0, 2, 1).contiguous()  # B*len_nun_max k

        obj_features_reshape = obj_feats.view(
    ...
    def _forward_sample_batch(self, data_dict, max_len=CONF.TRAIN.MAX_DES_LEN, min_iou=CONF.TRAIN.MIN_IOU_THRESHOLD, bert_embeds=None):
        """
        ...
        len_nun_max = data_dict["input_ids"].shape[1]
        batch_size, num_proposal = data_dict["bbox_feature"].shape[:2]
        # batch_size*lang_num_max, max_len, hidden
        input_ids = data_dict["input_ids"]
        word_embs = bert_embeds(input_ids)
        word_embs = word_embs.reshape(batch_size*len_nun_max, -1, 768)
        ...
        # batch_size, num_proposals, feat_size
        obj_feats = data_dict["bbox_feature"]
        ...
        obj_feats = feature1
        object_masks = object_masks[:, None, :].repeat(
            1, len_nun_max, 1).reshape(batch_size * len_nun_max, num_proposal)
        #################################caption########################################
        ...
        if self.use_oracle:
            ...
            target_ids, target_ious = select_multi_target(
                data_dict)  # batch_size * len_nun_max
        ...
        target_feats = torch.gather(obj_feats, 1, target_ids.view(
            batch_size*len_nun_max, 1, 1).repeat(1, 1, self.feat_size)).squeeze(1)  # batch_size * len_nun_max, emb_size
        ...
        valid_masks = object_masks if self.num_locals == - \
            1 else self._query_multi_locals(data_dict, target_ids, object_masks)
        ...
        if self.use_relation:
            obj_feats = self._add_relation_feat(
                data_dict, obj_feats, target_ids)
        ...
        outputs = []
        masks = []
        # batch_size*len_nun_max, hidden_size
        hidden = torch.zeros(batch_size * len_nun_max, self.hidden_size).cuda()
        step_id = 0
        ...
        while True:
            ...
            # batch_size*len_nun_max, num_vocabs
            step_output = self.classifier(hidden)

            # store
            # batch_size*len_nun_max, 1, num_vocabs
            ...
            if rand2 < 0.2:
                ...
                step_input = bert_embeds(unk).unsqueeze(0).repeat(batch_size * len_nun_max, 1)
            else:
            # batch_size*len_nun_max, emb_size
                step_input = word_embs[:, step_id]

            # RL Policy Gradient
            # TODO

        # batch_size*len_nun_max, num_words - 1/max_len, num_vocabs
        outputs = torch.cat(outputs, dim=1)
        # batch_size*len_nun_max, num_proposals, num_words - 1/max_len
    ...
    def _forward_scene_batch(self, data_dict, use_tf=False, max_len=CONF.TRAIN.MAX_DES_LEN, bert_embeds=None):
        """
        generate descriptions based on input tokens and object features
        """

        # unpack
        # word_embs = data_dict["lang_fea"]
        ...
        for prop_id in range(self.num_proposals):
            ...
            valid_prop_masks = object_masks if self.num_locals == - \
                1 else self._query_locals(data_dict, target_ids, object_masks)

            # object-to-object relation
            if self.use_relation:
                prop_obj_feats = self._add_relation_feat(
                    data_dict, prop_obj_feats, target_ids)
            ...
            while True:
                ...
                for batch_id in range(batch_size):
                    idx = step_output[batch_id].argmax()  # 0 ~ num_vocabs
                    # word = self.vocabulary["idx2word"][str(idx.item())]
                    # word = bert_embeds[idx]
                    # emb = torch.FloatTensor(self.embeddings[word]).unsqueeze(
                    #     0).cuda()  # 1, emb_size
                    emb = bert_embeds(idx)
                ...
                # store
                step_output = step_output.unsqueeze(
                    1)  # batch_size, 1, num_vocabs
            ...
            # batch_size, 1, num_proposals, num_words - 1/max_len
            prop_masks = torch.cat(prop_masks, dim=-1).unsqueeze(1)
            outputs.append(prop_outputs)
            masks.append(prop_masks)

        # batch_size, num_proposals, num_words - 1/max_len, num_vocabs
        outputs = torch.cat(outputs, dim=1)
        # batch_size, num_proposals, num_proposals, num_words - 1/max_len
        masks = torch.cat(masks, dim=1)
        # batch_size, num_proposals, num_proposals
