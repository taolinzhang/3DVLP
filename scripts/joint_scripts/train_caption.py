...
def get_dataloader(args, scanrefer, scanrefer_new, all_scene_list, split, config, augment, shuffle=True,
    ...
    # dataloader = DataLoader(dataset, batch_size=args.batch_size, shuffle=True, num_workers=2, prefetch_factor=4)
...
def get_model(args, dataset, device):
    ...
    if args.pretrain:
        ...
        for param in model.relation.parameters():
            param.requires_grad = False
        for param in model.lang.parameters():
            param.requires_grad = False
    # to CUDA
    model = model.cuda()
    return model
...
def get_scanrefer(args):
    if args.dataset == "ScanRefer":
        ...
        scanrefer_train = json.load(
            open(os.path.join(CONF.PATH.REFERIT3D, "nr3d_train.json")))
        scanrefer_eval_val = json.load(
            open(os.path.join(CONF.PATH.REFERIT3D, "nr3d_val.json")))
    elif args.dataset == "Sr3D":
        scanrefer_train = json.load(
            open(os.path.join(CONF.PATH.REFERIT3D, "sr3d_train.json")))
        scanrefer_eval_val = json.load(
            open(os.path.join(CONF.PATH.REFERIT3D, "sr3d_val.json")))
    elif args.dataset == "Sr3D_Plus":
        scanrefer_train = json.load(
            open(os.path.join(CONF.PATH.REFERIT3D, "sr3d_plus_train.json")))
        scanrefer_eval_val = json.load(
            open(os.path.join(CONF.PATH.REFERIT3D, "sr3d_plus_val.json")))
    elif args.dataset == "Nr3D_ScanRefer":
        scanrefer_train = json.load(
            open(os.path.join(CONF.PATH.REFERIT3D, "nr3d_scanrefer.json")))
        scanrefer_eval_val = json.load(
            open(os.path.join(CONF.PATH.REFERIT3D, "ScanRefer_filtered_val.json")))
    elif args.dataset == "Sr3D_ScanRefer":
        scanrefer_train = json.load(
            open(os.path.join(CONF.PATH.REFERIT3D, "sr3d_scanrefer.json")))
        scanrefer_eval_val = json.load(
            open(os.path.join(CONF.PATH.REFERIT3D, "ScanRefer_filtered_val.json")))
    else:
    ...
    print("train on {} samples from {} scenes".format(
        len(new_scanrefer_train), len(train_scene_list)))
...
def train(args):
    ...
    train_dataset, train_dataloader = get_dataloader(
        args, scanrefer_train, scanrefer_train_new, all_scene_list, "train", DC, True, SCAN2CAD_ROTATION)
    eval_train_dataset, eval_train_dataloader = get_dataloader(
        args, scanrefer_eval_train, scanrefer_eval_train_new, all_scene_list, "val", DC, False, shuffle=False)
    eval_val_dataset, eval_val_dataloader = get_dataloader(
        args, scanrefer_eval_val, scanrefer_eval_val_new, all_scene_list, "val", DC, False, shuffle=False)
    eval_val_dataset2, eval_val_dataloader2 = get_dataloader(
        args, scanrefer_eval_val2, scanrefer_eval_val_new2, all_scene_list, "val", DC, False, shuffle=False)
...
if __name__ == "__main__":
    ...
    wandb.init(project="A100", entity="3dplp",
               name="3djcg_caption_pretrain_mlm_6_freeze_s_lr_r")
    # wandb.init()
